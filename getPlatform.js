...
function isUnknownDevice(){
  // ブラウザ、OSが不明か判定
  //!null-->true
  const isNull_browser = !platform.name;
  const isNull_os = !platform.os.family;
...
function checkResolution(){
  ...
  const my_width = (is_undefined) ? 0 : screen.width;
  const my_height = (is_undefined) ? 0 : screen.height;
  const my_pair = [my_width, my_height];
